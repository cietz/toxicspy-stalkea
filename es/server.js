...
// ============ TRANSFERÊNCIA BANCÁRIA ARGENTINA ============

// Criar tabela de transações de transferência se não existir
async function initTransferTable() {
  try {
    await turso.execute(`
    ...
    `);
    console.log("✅ Tabela 'bank_transfers' criada/verificada");
  } catch (error) {
    console.error("❌ Erro ao criar tabela bank_transfers:", error);
  }
}

initTransferTable();

// Criar transação de transferência bancária
app.post(
  "/api/credits/create-transfer",
  authenticateToken,
  async (req, res) => {
    try {
      const { credits, amount } = req.body;
      ...
      // Validar valores de pacotes en ARS
      const validPackages = {
        100: 5830,
        600: 15580,
        1500: 29230,
      ...
      await turso.execute({
        sql: `INSERT INTO bank_transfers (id, user_id, credits, amount, status, expires_at) 
            VALUES (?, ?, ?, ?, 'pending', ?)`,
        args: [transactionId, req.user.userId, credits, amount, expiresAt],
      });

      console.log(
        `💰 [TRANSFER] Transação criada: ${transactionId} para ${req.user.userId}`,
      );
      ...
      res.json({
        ...
        bankData: {
          alias: "stalker.pro",
          cvu: "0000085700243957108575",
          titular: "Yenifer Daiana Ortiz",
        },
      });
    } catch (error) {
      console.error("❌ [TRANSFER] Erro ao criar transação:", error);
      res.status(500).json({
        error: "Erro ao criar transação de transferência",
...
app.post("/api/credits/submit-proof", authenticateToken, async (req, res) => {
  try {
    const { transactionId, proofFile, fileName, fileType } = req.body;

    if (!transactionId || !proofFile) {
      return res.status(400).json({
        error: "TransactionId e proofFile são obrigatórios",
      });
    }

    // Verificar se a transação existe e pertence ao usuário
    const transferResult = await turso.execute({
      sql: "SELECT * FROM bank_transfers WHERE id = ? AND user_id = ?",
      args: [transactionId, req.user.userId],
    });
    ...
    if (transfer.status === "approved" || transfer.status === "paid") {
      return res.status(400).json({
        error: "Esta transação já foi aprovada",
      });
    }

    // Atualizar transação com comprovante
    await turso.execute({
      sql: `UPDATE bank_transfers 
            SET proof_file = ?, file_name = ?, file_type = ?, status = 'pending_approval', updated_at = CURRENT_TIMESTAMP 
            WHERE id = ?`,
      args: [
        proofFile,
    ...
    console.log(
      `📎 [TRANSFER] Comprovante enviado para transação ${transactionId}`,
    );

    // TODO: Aqui você pode adicionar notificação para admin revisar
    ...
    console.error("❌ [TRANSFER] Erro ao enviar comprovante:", error);
    res.status(500).json({
...
// Verificar status da transação de transferência
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // Verificar se é transação PIX ou transferência
      if (transactionId.startsWith("transfer_")) {
        // É transferência bancária
        const transferResult = await turso.execute({
          sql: "SELECT * FROM bank_transfers WHERE id = ? AND user_id = ?",
          args: [transactionId, req.user.userId],
        });
        ...
        const transfer = transferResult.rows[0];
        const isPaid =
          transfer.status === "approved" || transfer.status === "paid";

        // Se foi aprovado e ainda não creditado
        if (isPaid) {
          const userResult = await turso.execute({
            sql: "SELECT metadata FROM users WHERE id = ?",
            args: [req.user.userId],
          ...
          if (!currentMetadata.includes(transactionId)) {
            // Adicionar créditos
            await turso.execute({
              sql: "UPDATE users SET credits = credits + ?, metadata = ? WHERE id = ?",
              args: [
                transfer.credits,
                currentMetadata + "," + transactionId,
                req.user.userId,
              ],
            });

            console.log(
              `✅ [TRANSFER] ${transfer.credits} créditos adicionados para usuário ${req.user.userId}`,
            );
          }
        }
        ...
        // É transação PIX - código original
        const auth = Buffer.from(
          `${MARCHABB_PUBLIC_KEY}:${MARCHABB_SECRET_KEY}`,
        ).toString("base64");
